...
def activate(request, uidb64, token):
    ...
    if user is not None and default_token_generator.check_token(user, token):
        ...
        redirect_url = 'http://localhost:5173/login/' + '?message=' + message + '&token=' + token  # Fix the URL format
    else:
...
def reset_validate(request, uidb64, token):
    ...
    if user is not None and default_token_generator.check_token(user, token):
        return HttpResponseRedirect(f'http://localhost:5173/resetpassword/')  # Corrected the URL format
